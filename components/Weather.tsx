...
export default function WeatherTile() {
  ...
  useEffect(() => {
    const fetchWeather = (lat: number, lon: number) => {
      const url = `http://api.weatherstack.com/current?access_key=cfade38566792df37a2b3600527dd007&query=${lat},${lon}`;
  ...
  if (!data)
    return (
      <div className="glass-card font-bold text-slate-700 p-2.5 w-24 h-24 fixed top-2 right-3 z-50 rounded-3xl shadow-lg hover:shadow-xl transition-shadow animate-none flex flex-col items-center justify-center bg-white/70">
        Loading Weather...
      </div>
    );
