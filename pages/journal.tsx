...
async function getJournalEntries(): Promise<Array<{ id: number; text: string; date: string }>> {
  ...
  return [
    { id: 1, text: "Switched to LED lighting", date: new Date().toLocaleString() },
    { id: 2, text: "Used public transport", date: new Date().toLocaleString() },
  ];
}

async function addJournalEntry(entry: { text: string; date: string }): Promise<{ id: number; text: string; date: string }> {
